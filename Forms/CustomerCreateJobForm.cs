...
{
    public partial class CustomerCreateJobForm : MaterialForm
    {
        private readonly JobRepository _jobRepository;
        private readonly CustomerRepository _customerRepository;
        private readonly PriceRangeRepository _priceRangeRepository; // Added repository
        ...
        private MaterialComboBox cmbVolume; // Changed from MaterialTextBox
        private MaterialTextBox txtJobDescription;
        private MaterialComboBox cmbPriority;
        private MaterialButton btnSave;
        private MaterialButton btnCancel;

        public CustomerCreateJobForm(int? customerId = null)
        {
            _jobRepository = new JobRepository();
            _customerRepository = new CustomerRepository();
            _priceRangeRepository = new PriceRangeRepository(); // Instantiate the repository
            ...
            LoadVolumePriceRanges(); // Load the price ranges for the volume
        }
        ...
        private void InitializeComponent()
        {
            ...
            cmbVolume = new MaterialComboBox
            {
            ...
            cardJobDetails.Controls.Add(cmbVolume);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                var selectedVolume = (dynamic)cmbVolume.SelectedItem;
                ...
                {
                    job.TotalEstimatedVolume = selectedVolume.Value;
                }
        ...
        private void LoadCustomers(int? customerIdToSelect)
        {
            try
            {
                // Ensure a customer ID is provided.
                if (customerIdToSelect.HasValue)
                {
                    // Fetch only the current customer from the repository.
                    var customer = _customerRepository.GetCustomerById(customerIdToSelect.Value);

                    if (customer != null)
                    {
                        // Create a list containing only the current customer.
                        var customerList = new List<object>
                {
                    new { Text = customer.FullName, Value = customer.CustomerId }
                };

                        // Bind the single-customer list to the ComboBox.
                        cmbCustomer.DataSource = customerList;
                        cmbCustomer.DisplayMember = "Text";
                        cmbCustomer.ValueMember = "Value";
                        cmbCustomer.SelectedValue = customerIdToSelect.Value;

                        // Disable the ComboBox to prevent the user from changing the selection.
                        cmbCustomer.Enabled = false;
                    }
                    else
                    {
                        MaterialMessageBox.Show("Could not load customer details.", "Error",
                            MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
                else
                {
                    // Handle the case where no customer is logged in or identified.
                    MaterialMessageBox.Show("No customer is selected. Please log in again.", "Authentication Error",
                        MessageBoxButtons.OK, MessageBoxIcon.Error);
                    this.Close(); // Close the form if no customer is identified.
                }
            }
            catch (Exception ex)
            {
                MaterialMessageBox.Show($"Error loading customer: {ex.Message}", "Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private void LoadVolumePriceRanges()
        {
            try
            {
                var priceRanges = _priceRangeRepository.GetAll();
                var volumeList = priceRanges.Select(pr => new { Text = $"{pr.FromWeight} - {pr.ToWeight} kg", Value = pr.Price }).ToList();

                cmbVolume.DataSource = volumeList;
                cmbVolume.DisplayMember = "Text";
                cmbVolume.ValueMember = "Value";
            }
            catch (Exception ex)
            {
                MaterialMessageBox.Show($"Error loading volume options: {ex.Message}", "Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private bool ValidateForm()
        {
            ...
            {
                MaterialMessageBox.Show("Please select a volume.", "Validation Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                cmbVolume.Focus();
                return false;
            }
